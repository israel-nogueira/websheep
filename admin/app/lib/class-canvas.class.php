...
class canvas {
	...
	*/
	public function grava( $destino='', $qualidade = 100 )
	{
		...
		switch( $extensao_destino )
		{
			...
			case 'bmp':
				if ( $destino )
				{
					if(imagejpeg( $this->img, $destino, $qualidade ))return true;exit;
				}
				else
				{
			...
			case 'png':
				if ( $destino )
				{
					if(imagepng( $this->img, $destino ))return true;exit;
				}
				else
				{
			...
			case 'gif':
				if ( $destino )
				{
					if(imagegif( $this->img, $destino ))return true;exit;
				}
				else
				{
	 ...
	 * @return void
	*/
	public function exibe( $destino=''){
		if ($this->extensao == 'jpg' || $this->extensao == 'bmp') {
			header("Content-type: image/jpeg");
			imagejpeg( $this->img, NULL, $qualidade );
			readfile($this->img);
			imagedestroy($this->img );
		} elseif ($this->extensao == 'png') {
			header("Content-type: image/png");
			imagepng( $this->img);
			readfile($this->img);
